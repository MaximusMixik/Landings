(() => {
    ...
    function initSliders() {
        if (document.querySelector(".drivers-swiper")) new swiper_core_Swiper(".drivers-swiper", {
            ...
            speed: 800,
            pagination: {
                el: ".drivers-swiper .navigation__pagination",
                clickable: true
            },
            navigation: {
                prevEl: ".drivers-swiper .navigation__button--prev",
                nextEl: ".drivers-swiper .navigation__button--next"
            }
        });
        ...
        if (window.innerWidth > 768 && document.querySelector(".cargo-swiper")) new swiper_core_Swiper(".cargo-swiper", {
 ...
 */
    ...
    if (typeof ScrollTrigger_ScrollTrigger === "function") gsapWithCSS.registerPlugin(ScrollTrigger_ScrollTrigger); else console.error("ScrollTrigger plugin not loaded properly");
    if (typeof SplitText === "function") gsapWithCSS.registerPlugin(SplitText); else console.error("SplitText plugin not loaded properly");
    ...
    function initHeroAnimationOnLoad() {
        ...
        const animateElements = [ {
            ...
            selector: ".hero__about",
            delay: .6
        }, {
            selector: ".hero__button-wrap",
            delay: .8
        }, {
            selector: ".hero__text",
            delay: 1
        ...
        animateElements.forEach((item => {
            const elements = document.querySelectorAll(`.hero ${item.selector}`);
            gsapWithCSS.set(elements, {
                opacity: 0,
                y: 30
            });
        }));
        const heroItems = document.querySelectorAll(".hero__item.item-hero");
        ...
        const bgImages = document.querySelectorAll(".hero__bg, .hero__bg-mob");
        gsapWithCSS.set(bgImages, {
            opacity: 0,
            scale: 1.1
        });
        heroTl.to(bgImages, {
        ...
        animateElements.forEach((item => {
            const elements = document.querySelectorAll(`.hero ${item.selector}`);
            if (elements.length > 0) if (item.stagger) heroTl.to(elements, {
                opacity: 1,
                y: 0,
                duration: .6,
                ease: "power3.out",
                stagger: item.stagger
            }, item.delay); else heroTl.to(elements, {
                opacity: 1,
                y: 0,
                duration: .6,
                ease: "power3.out"
    ...
    const createElm = function(menuItem) {
        ...
        const originalText = menuLink.textContent;
        const lettersArray = [ ...originalText ];
        menuLink.textContent = "";
        const wrapper = document.createElement("div");
        wrapper.className = "menu__text-wrapper";
        const firstSet = document.createElement("div");
        firstSet.className = "menu__letters-set";
        lettersArray.forEach((letter => {
            const span = document.createElement("span");
            span.className = "menu__letter";
            span.textContent = letter === " " ? " " : letter;
            firstSet.appendChild(span);
        }));
        wrapper.appendChild(firstSet);
        menuLink.appendChild(wrapper);
        const wrapperHeight = wrapper.clientHeight;
        wrapper.style.height = `${wrapperHeight}px`;
        const cloneSet = firstSet.cloneNode(true);
        cloneSet.className = "menu__letters-set menu__letters-clone";
        wrapper.appendChild(cloneSet);
    };
    const animation = function(menuItem) {
        ...
        const wrapper = menuItem.querySelector(".menu__text-wrapper");
        const originalLetters = wrapper.children[0].children;
        const cloneLetters = wrapper.children[1].children;
        menuItem.addEventListener("mouseenter", (function() {
            gsapWithCSS.to(originalLetters, {
                y: "-100%",
                stagger: {
                    amount: .14,
                    from: "start"
            ...
            gsapWithCSS.to(menuItem.querySelector(".menu__link"), {
                color: "var(--accent)",
                duration: .3
            });
        }));
    ...
    const initGSAPMenuAnimation = function() {
        const targetItems = document.querySelectorAll(".menu__item");
    ...
    function initWithRetry() {
        ...
        function tryInit() {
            ...
            try {
                ...
                if (window.innerWidth > 768) initGSAPMenuAnimation();
            } catch (error) {
    ...
    setTimeout((() => {
        ...
        if (processedElements.length === 0) initWithRetry();
    }), 2e3);
    class navigation_Navigation {
        ...
        setupMenuLinks() {
            ...
            menuLinks.forEach((link => {
                link.addEventListener("click", (() => {
                    ...
                    if (anchor) sessionStorage.setItem(this.settings.storageKey, anchor);
        ...
        scrollTo(elementId, saveToStorage = false) {
            if (saveToStorage) sessionStorage.setItem(this.settings.storageKey, elementId);
